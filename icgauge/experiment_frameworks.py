...
from icgauge import feature_extractors as fe
from icgauge import label_transformers as lt
from icgauge import training_functions as training
    ...
    for i, (paragraph, parse, label) in enumerate(reader()):
        ...
        if cls != None:
            ...
            for phi in phi_list:
                ...
                overlap_feature_names = features.keys() & cur_feats.keys()
...
def experiment_features(
    ...
    # Train dataset:
    train = build_dataset(
        train_reader, phi_list, class_func, vectorizer=None, verbose=verbose
    )
    ...
    if assess_reader == None:
        ...
        assess = build_dataset(
            assess_reader, phi_list, class_func, vectorizer=train["vectorizer"]
        )
        X_assess, y_assess, assess_examples = (
            assess["X"],
            assess["y"],
            np.array(assess["raw_examples"]),
        )

    # Normalize:
    nonzero_cells = len(X_train.nonzero()[0])
    total_cells = 1.0 * X_train.shape[0] * X_train.shape[1]
    ...
    assess_performance = get_score_example_pairs(
        y_assess, predictions_on_assess, assess_examples
    )

    predictions_on_train = mod.predict(X_train)
    train_performance = get_score_example_pairs(
        y_train, predictions_on_train, train_examples
    )
...
def get_score_example_pairs(y, y_hat, examples):
    ...
    for i, (truth, prediction) in enumerate(paired_results):
        performance.append(
            {"truth": truth, "prediction": prediction, "example": examples[i]}
        )
    return performance
...
def experiment_features_iterated(
    ...
    while len(correlation_overall) < iterations:
        ...
        try:
            correlation_local, cronbach_local, conf_matrix_local, perf_local = (
                experiment_features(
