...
def semantic_orientation(
    ...
    scores = [
        (rownames[i], _so_row_func(mat[i], sm1, sm2, distfunc)) for i in range(len(mat))
    ]
...
def _so_seed_matrix(seeds, mat, rownames):
    ...
    if not indices:
        raise ValueError(
            "The matrix contains no members of the seed set: %s" % ",".join(seeds)
        )
...
def men_reader():
    ...
    src_filename = os.path.join(
        vsmdata_home, "wordsim", "MEN_dataset_natural_form_full"
    )
...
def ttest(mat, rownames=None):
    ...
    p = np.multiply((p - intersection), 1.0 / np.sqrt(intersection))
    return (p, rownames)
...
def semantic_orientation_lexicon(mat, rownames, posset, negset, distfunc=cosine):
    ...
    ranking = semantic_orientation(
        mat=mat, rownames=rownames, seeds1=negset, seeds2=posset
    )
...
def evaluate_semantic_orientation_lexicon(lexicon):
    ...
    assessment_lexicon = pickle.load(
        file(os.path.join(vsmdata_home, "imdb-posneg-lexicon.pickle"))
    )
