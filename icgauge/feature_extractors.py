...
from icgauge import utils_wordlists
from icgauge import utils_parsing
from icgauge import utils_similarity
from icgauge.utils_vsm import semantic_orientation_score_lexicon
...
if os.path.exists(os.path.join("data/", "semantic_lexicon.pickle")):
    semantic_lexicon = pickle.load(
        file(os.path.join("data/", "semantic_lexicon.pickle"))
    )
...
if os.path.exists(os.path.join("data/", "lstm_representation_dict")):
    lstm_representation_dict = pickle.load(
        file(os.path.join("data/", "lstm_representation_dict"))
    )
...
def baseline_features(paragraph, parse):
    return manual_content_flags(paragraph, parse) | baseline_length(paragraph, parse)
...
def all_features(paragraph, parse):
    return (
        baseline_features(paragraph, parse)
        | lexical_features(paragraph, parse)
        | length(paragraph, parse)
        | syntactic_features(paragraph, parse)
...
def lexical_features(paragraph, parse):
    return (
        get_morphological_counts(paragraph, parse)
...
def syntactic_features(paragraph, parse):
    return determiner_usage(paragraph, parse) | syntactic_parse_features(
        paragraph, parse
    )
...
def word_intensity(paragraph, unused_parse):
    ...
    if semantic_lexicon == None:
        glv = utils.build_glove(
            os.path.join(path_to_glove, "glove.6B.%dd.txt" % GLOVE_SIZE)
        )
        semantic_lexicon = semantic_orientation_score_lexicon(
            mat=glv[0],
            rownames=glv[1],
            negset=(
                "little",
    ...
    for word in tokenized_and_lowercase:
        ...
        if (not punctuated and len(sentence) >= AVG_SENTENCE_LENGTH) or (
            punctuated and word in [".", "?", "!"]
        ):
            m = np.mean(sentence)
...
def manual_content_flags(paragraph, unused_parse):
    ...
    """
    flags = {
        "flag_1": utils_wordlists.get_manual_flags(1),
    ...
    tokenized_and_lowercase = word_tokenize(paragraph.lower())
    for label, set_of_flags in flags.items():
...
def length(paragraph, unused_parse):
    ...
    result["length_median_word_len"] = np.median(
        word_lengths
    )  # equiv to median_frequency
...
def dimensional_decomposition_noncumulative(
    paragraph, unused_parse, num_dimensions_to_accumulate=50
):
...
def syntactic_parse_features(paragraph, parse):
    ...
    features = dict(
        ("syntactic_head_" + key, counts_of_heads[key])
        for key in counts_of_heads
        if key in KEPT_FEATURES
    )
...
def baseline_length(paragraph, unused_parse):
    ...
    return keep_only(
        length(paragraph, unused_parse), ["length_in_words", "num_words_greater_than_6"]
    )
...
def modal_definite_presence(paragraph, unused_parse):
    ...
    modals["modal_def_count_token"] = np.sum([value for key, value in modals.items()])
...
def modal_indefinite_presence(paragraph, unused_parse):
    ...
    modals["modal_indef_count_token"] = np.sum([value for key, value in modals.items()])
...
def transitional_presence(paragraph, unused_parse):
    ...
    transitional["transitional_count_type"] = (
        len(transitional) - 1
    )  # -1 bc *_count_token
    return transitional
...
def hedge_presence(paragraph, unused_parse):
    ...
    hedges["hedge_count_token"] = np.sum([value for key, value in hedges.items()])
...
def conjunctives_presence(paragraph, unused_parse):
    ...
    conjunctives["conjunctive_count_type"] = (
        len(conjunctives) - 1
    )  # -1 bc *_count_token
    return conjunctives
...
def punctuation_presence(paragraph, unused_parse):
    ...
    result["punctuation_count_token"] = np.sum([value for key, value in result.items()])
