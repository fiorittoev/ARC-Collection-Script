        ...
        for fn in os.listdir(dirname):
            if fn.endswith(".json"):
                ...
                with open(os.path.join(dirname, fn)) as json_file:
                    ...
                    for item in dataset:
                        self.assertIn(
                            "paragraph", item, "The `paragraph` property is missing"
                        )
                        self.assertIsNotNone(
                            item["paragraph"], "A `paragraph` content is missing"
                        )
                        self.assertGreater(
                            len(item["paragraph"]),
                            0,
                            "A `paragraph` content has no length",
                        )

                        if "score" in item:
                            self.assertIsNotNone(
                                item["score"], "A `score` content is missing"
                            )
                            self.assertIn(
                                item["score"],
                                [1, 2, 3, 4, 5, 6, 7, "NA"],
                                "A `score` value is invalid",
                            )
            ...
            if fn.endswith(".json"):
                ...
                with open(os.path.join(dirname, fn)) as json_file:
                    for line in json_file:
                        for c in line:
                            self.assertTrue(
                                ord(c) < 128, "File contains a non-ASCII character"
                            )
